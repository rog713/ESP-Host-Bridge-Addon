...
function getApiUrl(path) {
  const root = window.__HOST_METRICS_INGRESS_PATH__ || '';
  return root + path;
}
...
async function pollLogs() {
  try {
    const r = await fetch(getApiUrl(`/api/logs?since=${nextLogId}`));
...
async function pollCommLogs() {
  try {
    const r = await fetch(getApiUrl(`/api/comm-logs?since=${nextCommLogId}`));
    const data = await r.json();
...
async function fetchHardwareChoices() {
  const r = await fetch(getApiUrl('/api/hardware-choices'));
  return await r.json();
}
...
async function refreshSerialPorts() {
  ...
  btn.disabled = true;
  try {
    const r = await fetch(getApiUrl('/api/ports'));
    const data = await r.json();
...
async function testSerialPort() {
  ...
  btn.disabled = true;
  try {
    const r = await fetch(getApiUrl('/api/test-serial'), {
      method: 'POST',
...
async function clearLogs() {
  ...
  btn.disabled = true;
  try {
    const r = await fetch(getApiUrl('/api/logs/clear'), { method: 'POST' });
    if (r.ok) {
      mainLogRows = [];
      renderMainLogs();
      nextLogId = 1;
...
function downloadLogs() {
  window.location.href = getApiUrl('/api/logs/text');
}
...
async function clearCommLogs() {
  ...
  btn.disabled = true;
  try {
    const r = await fetch(getApiUrl('/api/comm-logs/clear'), { method: 'POST' });
    if (r.ok) {
...
function downloadCommLogs() {
  window.location.href = getApiUrl('/api/comm-logs/text');
}
